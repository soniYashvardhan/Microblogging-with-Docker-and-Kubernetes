...
user = 'fullMarks'           # username as set for the mongodb admin server (the username used in secret.yaml - before base64 conversion)
password = 'apple123'       # password as set for the mongodb admin server (the password used in secret.yaml - before base64 conversion)
...
def home():
    ...
    if(len(posts)==0):
        ...
        db.posts.insert_one({"title": "mein kumpf", "author": "Fuhrer", "createdAt": datetime.now()})
        db.posts.insert_one({"title": "BJP", "author": "Amit Kaka", "createdAt": datetime.now()})
